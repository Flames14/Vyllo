...
object TTMLParser {
    ...
    fun parseTTML(ttml: String): List<ParsedLine> {
        ...
        try {
            ...
            SecureLogger.e(TAG, "Failed to parse TTML: ${e.message}", e)
        }
        return lines
    }
    ...
    private fun timingAttr(el: Element, localName: String): String {
        val direct = el.getAttribute(localName)
        if (direct.isNotEmpty()) return direct
        val param = el.getAttributeNS("http://www.w3.org/ns/ttml#parameter", localName)
    ...
    private fun parseP(p: Element, lines: MutableList<ParsedLine>, offset: Double, divAgent: String?) {
        ...
        if (begin.isEmpty()) {
            begin = p.getAttributeNS("http://www.w3.org/ns/ttml#parameter", "begin")
        }
        if (begin.isEmpty()) {
    ...
    private fun mergeSpansIntoWords(spanInfos: List<SpanInfo>): List<ParsedWord> {
        ...
        var text = java.lang.StringBuilder(spanInfos[0].text)
        ...
        for (i in 1 until spanInfos.size) {
            ...
            if (prev.hasTrailingSpace && !prev.text.endsWith('-')) {
                ...
                text = java.lang.StringBuilder(curr.text)
    ...
    fun toLRC(lines: List<ParsedLine>): String {
        ...
        val sb = java.lang.StringBuilder(lines.size * 128)
        var lastBg = false
        lines.forEach { line ->
